    ...
    for batch_idx, data in enumerate(tqdm(loader)):
        ...
        with autocast():
            ...
            if len(device.gpu_list)>1:
                loss = model.module.criterion_calculation(ret_dict, label, label_lgt) + ret_dict['ssim_loss']
            else:
                loss = model.criterion_calculation(ret_dict, label, label_lgt) + ret_dict['ssim_loss']
...
def seq_eval(cfg, loader, model, device, mode, epoch, work_dir, recoder, evaluate_tool="python"):
    ...
    recoder.print_log('\tEpoch: {} {} done. ST wer: {:.4f}  ins:{:.4f}, del:{:.4f}'.format(
        epoch, mode, wer_results_con['wer'], wer_results_con['ins'], wer_results_con['del']),
        f"{work_dir}/{mode}.txt")
    recoder.print_log('\tEpoch: {} {} done. LT wer: {:.4f}  ins:{:.4f}, del:{:.4f}'.format(
